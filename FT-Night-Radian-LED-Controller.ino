...
void updateShowConfig() { // sets order of currently active shows. e.g., activeShowNumbers[] = {1, 4, 5, 9}. also sets nav stop point.
  ...
  if (config.navlights) {
    ...
    fuseNavPoint = FUSE_LEDS - FUSE_NAV_LEDS;
    Serial.println(F("on."));
  } else {
    wingNavPoint = WING_LEDS;
    fuseNavPoint = FUSE_LEDS;
...
void setColor (CRGB color) { // sets all LEDs to a solid color
  ...
  fill_solid(fuseleds, fuseNavPoint, color);
...
void setColor (CRGBPalette16 palette) { // spreads a palette across all LEDs
  for (int i; i < wingNavPoint; i++) {
    ...
    if (i < fuseNavPoint) {fuseleds[i] = ColorFromPalette(palette, map(i, 0, fuseNavPoint, 0, 240));}
...
void chase(CRGB color1, CRGB color2, uint8_t speedWing, uint8_t speedFuse, uint8_t speedTail, bool cylon) {
  if (color2 == (CRGB)CRGB::Black) {
    ...
    for (uint8_t i = 0; i < fuseNavPoint; i++) {fuseleds[i] = fuseleds[i].lerp8(color2, 20);}
...
void setNavLeds(const struct CRGB& rcolor, const struct CRGB& lcolor) { // helper function for the nav lights
  ...
  for (uint8_t i = fuseNavPoint; i < FUSE_LEDS; i++) {
  ...
  switch(style) {
    ...
    case 1: //Rapid strobing all LEDS in unison
      if (StrobeState) {
        ...
        for (int i = 0; i < fuseNavPoint; i++) {fuseleds[i] = CRGB::White;}
        ...
        for (int i = 0; i < fuseNavPoint; i++) {fuseleds[i] = CRGB::Black;}
    ...
    case 2: //Alternate strobing of left and right wing
      if (StrobeState) {
        ...
        for (int i = 0; i < fuseNavPoint; i++) {fuseleds[i] = CRGB::Blue;}
        for (int i = 0; i < TAIL_LEDS; i++) {tailleds[i] = CRGB::White;}
      } else {
        for (int i = 0; i < wingNavPoint; i++) {
          rightleds[i] = CRGB::Black;
        ...
        for (int i = 0; i < fuseNavPoint; i++) {fuseleds[i] = CRGB::Yellow;}
        for (int i = 0; i < TAIL_LEDS; i++) {tailleds[i] = CRGB::White;}
      }
      interval = 500;
...
void altitude(double fake, CRGBPalette16 palette) { // Altitude indicator show. wings fill up to indicate altitude, tail goes green/red as variometer
  ...
  for (int i=0; i < map(currentAlt, 0, MAX_ALTIMETER, 0, fuseNavPoint); i++) {
    fuseleds[i] = CRGB::White;
  }
  for (int i=map(currentAlt, 0, MAX_ALTIMETER, 0, fuseNavPoint); i < fuseNavPoint; i++) {
...
void twinkle1 () { // Random twinkle effect on all LEDs
  ...
  doTwinkle1(fuseleds,  pixelStateFuse, fuseNavPoint);
...
void programInit(char progState) { // flashes red/green/white for different program mode indicators
  ...
  for (int j = 0; j < 7; j++) {
      if (StrobeState) {
        ...
        for (int i = 0; i < fuseNavPoint; i++) {fuseleds[i] = color;}
